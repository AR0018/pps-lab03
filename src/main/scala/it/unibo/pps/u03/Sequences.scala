...
//      def tailAdd(s: Sequence[A])(el: A): Sequence[A] = s match
//        case Cons(h, Nil()) => Cons(h, Cons(el, Nil()))
//        case Cons(h, t) => Cons(h, tailAdd(t)(el))
//        case _ => Cons(el, Nil())
//      s match
//      case Cons(h, t) => tailAdd(reverse(t))(h)
//      case _ => Nil()
     ...
     * E.g., [] => []
     */
    // Original version with concat, but no tail recursion
//    def reverse[A](s: Sequence[A]): Sequence[A] =
//      s match
...
//        case _ => Nil()
    ...
    def distinct[A](s: Sequence[A]): Sequence[A] =
      ...
      def _distinct(s: Sequence[A], distinctVals: Sequence[A]): Sequence[A] = s match
        case Cons(h, t) if contains(distinctVals)(h) => _distinct(t, distinctVals)
        case Cons(h, t) => _distinct(s, concat(distinctVals, Cons(h, Nil())))
        case _ => distinctVals
      _distinct(s, Nil())
